...
const CSS_STYLES = `
  ...
  .editor-content > h5, 
  .editor-content > h6 {
    margin-top: 0.1em !important;  /* 标题上方保留一定间距 */
    margin-bottom: 0.1em !important; /* 标题下方间距减小 */
  ...
  .editor-content > ol {
    ...
    margin-bottom: 0em !important; /* 列表整体间距减小 */
  ...
  .editor-content > ol li { 
    margin-bottom: 3px !important; /* 修复语法错误：3pxem → 3px */
    line-height: 1 !important;
  }
  
  .editor-content > pre { 
    padding: 0.2em !important; 
    margin: 0 0 !important; /* 代码块间距适中 */
    line-height: 1.5 !important;
  }
  ...
  .resizing-body .resizer { cursor: col-resize !important; } /* 强制显示调整光标 */
  ...
  .column {
    ...
    overflow: auto; display: flex; flex-direction: column; transition: flex-basis 0.05s linear; /* 加速过渡 */
  }
  ...
  .resizer {
    ...
    display: flex; align-items: center; justify-content: center; z-index: 20; /* 提高层级避免遮挡 */
  }
  .resizer .inner-bar {
    width: 4px; height: 80px; background: #6366f1; border-radius: 2px;
    transition: background 0.2s;
  }
  .resizer:hover .inner-bar { background: #4f46e5; } /* 悬停反馈 */
...
/**
 * 多栏编辑器组件（JSON文件存储，按笔记独立管理）
 */
class MultiColumnEditor {
    constructor(container) {
        this.container = container;
        this.dataFilePath = '笔记/脚本/multi-column-data.json'; // 存储文件路径
        this.state = {
            columns: [],
        ...
        this.endResizeBound = this.endResize.bind(this);
    }
     ...
     */
    getNoteUniqueId() {
        return dv.current().file.path.replace(/\//g, '_').replace(/\./g, '-');
    }
    ...
    async loadDataFile() {
        try {
            const file = app.vault.getAbstractFileByPath(this.dataFilePath);
            if (!file) {
                // 文件不存在时创建初始结构
                const initialData = { notes: {} };
                await app.vault.create(this.dataFilePath, JSON.stringify(initialData, null, 2));
                return initialData;
            }
    ...
    async saveDataFile(data) {
        try {
            const file = app.vault.getAbstractFileByPath(this.dataFilePath);
            const formattedData = JSON.stringify(data, null, 2); // 格式化JSON便于阅读
            if (!file) {
                await app.vault.create(this.dataFilePath, formattedData);
            } else {
                await app.vault.modify(file, formattedData);
            }
        } catch (e) {
    ...
    createColumnElement(data) {
        ...
        column.innerHTML = `
            ...
            <div class="editor-content" style="flex-grow: 1; min-height: 350px; outline: none; font-size: 15px; line-height: 1.6; color: #374151; white-space: pre-wrap; word-break: normal; padding: 0 3px;" contenteditable="true" data-markdown="${this.escapeHtml(data.content || '')}">${renderedContent}</div>
        `;
        return column;
    }
    ...
    async loadColumnsData() {
        try {
            ...
            const noteId = this.getNoteUniqueId();
            return data.notes[noteId]?.columns || null;
        } catch (e) {
            console.error('加载分栏数据失败:', e);
            return null;
        }
    }
    ...
    async saveColumnsData() {
        ...
        try {
            ...
            const columnsData = currentColumns.map((col, index) => ({
            ...
            const data = await this.loadDataFile();
            const noteId = this.getNoteUniqueId();
            data.notes[noteId] = { columns: columnsData };

            await this.saveDataFile(data);
        } catch (e) {
            console.error('保存分栏数据失败:', e);
        } finally {
    ...
    handleColumnContainerBlur(e) {
        if (e.target.classList.contains('editor-content') && e.target.classList.contains('editing')) {
            ...
            this.saveColumnsData(); // 改为异步，但无需await（不阻塞UI）
        }
    }
    ...
    startResize(e, resizer) {
        ...
        this.state.containerWidth = this.elements.columnContainer.getBoundingClientRect().width;
        this.state.isResizing = true;
        this.state.startX = e.clientX;
        this.state.initialLeftWidth = parseFloat(this.state.leftColEl.style.flexBasis || (100 / this.getColumnCount()));
        this.state.initialRightWidth = parseFloat(this.state.rightColEl.style.flexBasis || (100 / this.getColumnCount()));

        document.body.classList.add('resizing-body');
        document.addEventListener('mousemove', this.doResizeBound);
        document.addEventListener('mouseup', this.endResizeBound);
    ...
    doResize(e) {
        ...
        const percentDelta = (mouseDelta / this.state.containerWidth) * 100;
    ...
    getColumnCount() {
        return Array.from(this.elements.columnContainer.querySelectorAll('.column')).length;
    }
    ...
    addNewColumn(data = {}) {
        ...
        // 修复空状态残留：清除空状态提示
        if (this.elements.columnContainer.querySelector('.empty-state')) {
            this.elements.columnContainer.innerHTML = '';
        }
    ...
    async deleteColumn(column) {
        ...
        if (columns.length <= 1) {
            confirmMessage = '确定要删除最后一个分栏吗？此操作将清除当前笔记的分栏数据！';
        }
    ...
    async init() {
        try {
            ...
            const savedData = await this.loadColumnsData(); // 异步加载数据
...
if (typeof dv !== 'undefined' && typeof app !== 'undefined') {
