---
title: 未命名 1
updateDate: 2025-11-25 10:05:36
---
...
// 修复：单独同步指定分栏的内容
const syncSingleColumnContent = (columnEl) => {
    const editor = columnEl.querySelector('.editor-content');
    if (editor && editor.classList.contains('editing')) {
        const cleanedContent = editor.innerText.replace(/\n{3,}/g, '\n\n');
        editor.dataset.markdown = cleanedContent;
        editor.classList.remove('editing');
...
const createColumnElement = (title = '', content = '', width = '') => {
    ...
    newColumn.style.cssText = `flex: 1 1 ${width}; padding: 15px; background: #f9fafb; box-sizing: border-box; overflow: auto; display: flex; flex-direction: column;`;
    ...
    newColumn.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center; font-size: 18px; font-weight: 600; margin-bottom: 12px; padding-bottom: 8px; border-bottom: 2px solid #6366f1;">
            <span class="column-title preview" contenteditable="true" spellcheck="false">${title}</span>
        ...
        <div class="editor-content preview" style="
            flex-grow: 1; 
            min-height: 350px; 
            outline: none; 
            font-size: 15px; 
            ...
            padding: 4px 0;
        " contenteditable="true" data-markdown="${content.replace(/"/g, '&quot;')}">
...
const bindColumnEvents = (columnEl) => {
    ...
    // 修复分栏标题点击事件
    columnTitle.addEventListener('focus', (e) => {
        columnTitle.classList.add('editing');
        columnTitle.classList.remove('preview');
    });

    columnTitle.addEventListener('blur', (e) => {
        columnTitle.classList.remove('editing');
        columnTitle.classList.add('preview');
        saveColumnsData();
    });
    ...
    editorContent.addEventListener('click', (e) => {
        if (!editorContent.classList.contains('editing')) {
            ...
            columns.forEach(col => {
                if (col !== columnEl) {
                    syncSingleColumnContent(col);
                }
            });
            
            // 进入编辑模式
            editorContent.classList.add('editing');
            editorContent.classList.remove('preview');
        ...
        if (editorContent.classList.contains('editing')) {
            setTimeout(() => {
                ...
                if (document.activeElement !== editorContent) {
                    ...
                    editorContent.classList.add('preview');
