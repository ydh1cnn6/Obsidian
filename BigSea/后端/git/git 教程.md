...
# cherry-pick
场景一：A分支提交代码，合并到B分支(这种场景可以用merge直接合并)，可以用cherry-pick
场景二：A分支指定某一个或多个提交（某些提交不想合并到B），合并到B分支
	1)不冲突：直接合并
	2)冲突：（场景：其他提交在之前也修改了要提交的文件，AB的该文件在合并前就不一致）要手动merge，会有3个代码（分别是B分支的代码，合并结果，A分支的代码）
# patch
场景一：A分支提交代码，合并到B分支(这种场景可以用merge直接合并)，可以用patch
场景二：A分支指定某一个或多个提交（某些提交不想合并到B），合并到B分支
	1)不冲突：直接合并
	2)冲突：（场景：其他提交在之前也修改了要提交的文件，AB的该文件在合并前就不一致），会只应用该提交修改的内容，其他内容不管。（<font color="#ff0000">不会去解决冲突</font>）
[Cherry Pick与Create Patch的区别-CSDN博客](https://blog.csdn.net/qq_22326625/article/details/108030040)
